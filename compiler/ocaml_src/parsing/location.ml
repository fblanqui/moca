...
(* $Id: location.ml,v 1.6 2012-04-02 08:35:26 weis Exp $ *)
...
let highlight_terminfo ppf num_lines lb loc1 loc2 =
  Format.fprintf ppf "%!";  (* avoid mixing Format and normal output *)
...
let highlight_dumb ppf lb loc =
  ...
  for pos = 0 to end_pos do
    ...
    if c <> '\n' then begin
      ...
      if !line >= !line_start && !line <= !line_end then begin
        Format.fprintf ppf "@.";
        if pos < loc.loc_end.pos_cnum then Format.fprintf ppf "%s" "  "
      end;
      incr line;
      pos_at_bol := pos + 1;
    end
