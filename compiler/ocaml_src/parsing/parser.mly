...
/* $Id: parser.mly,v 1.57 2011-12-05 14:16:52 bonichon Exp $ */
...
module_expr:
  ...
  | LPAREN VAL expr COLON package_type RPAREN
      { mkmod (Pmod_parens (mkmod (Pmod_unpack($3, $5)))) }
...
simple_expr:
  ...
  | LPAREN seq_expr RPAREN
      { mkexp(Pexp_parens(reloc_exp $2)) }
  | LPAREN seq_expr error
      { unclosed "(" 1 ")" 3 }
  | BEGIN seq_expr END
      { mkexp (Pexp_begend (reloc_exp $2)) }
  | BEGIN END
  ...
  | LPAREN MODULE module_expr COLON package_type RPAREN
      { mkexp (Pexp_parens (mkexp (Pexp_pack ($3, $5)))) }
...
type_kind:
  ...
  | EQUAL core_type relations
      { (Ptype_abstract $3, Public, Some $2) }
  | EQUAL PRIVATE core_type relations
      { (Ptype_abstract $4, Private, Some $3) }
...
constructor_declaration:
    constr_ident constructor_arguments relations { ($1, $2, $3, symbol_rloc()) }
;
...
/* Relations for relational types */

relations:
  | /* empty */                          { mkrels Prels_none }
  | BEGIN relation_list END              { mkrels (Prels_begend (List.rev $2)) }
;
relation_list:
  | /* empty */                          { [] }
  | relation_list relation               { $2 :: $1 }
;
comparison_function:
  | /* empty */                          { None }
  | LPAREN val_longident RPAREN          { Some $2 }
;
relation:
  | RULE pattern match_action   { mkrel (Rewrite ($2, $3)) }
  | COMMUTATIVE comparison_function { mkrel (Commutative $2) }
  | ASSOCIATIVE                 { mkrel (Associative Left) }
  | ABSORBENT relation_side LPAREN generator RPAREN
                                { mkrel (Absorbent ($2, $4)) }
  | ABSORBING relation_side LPAREN generator RPAREN
                                { mkrel (Absorbing ($2, $4)) }
  | NEUTRAL relation_side LPAREN generator RPAREN
                                { mkrel (Neutral ($2, $4)) }
  | IDEMPOTENT relation_side    { mkrel (Idempotent $2) }
  | NILPOTENT relation_side LPAREN generator RPAREN
                                { mkrel (Nilpotent ($2, $4)) }
  | INVOLUTIVE                  { mkrel (Involutive) }
  | inverse relation_side LPAREN generator RPAREN
                                { mkrel (Inverse ($2, $4, None)) }
  | inverse relation_side LPAREN generator COMMA generator RPAREN
                                { mkrel (Inverse ($2, $4, Some $6)) }
  | DISTRIBUTIVE distributivity_direction
                 relation_side LPAREN generator RPAREN
                                { mkrel (Distributive ($3, $5, None, $2)) }
  | DISTRIBUTIVE distributivity_direction
                 relation_side LPAREN generator COMMA generator RPAREN
                                { mkrel (Distributive ($3, $5, Some $7, $2)) }
  | COMPLETION PRECEDENCE INT   { mkrel (Precedence $3) }
  | COMPLETION STATUS completion_status
                                { mkrel (Status $3)}
  | structure_item SEMISEMI     { mkrel (Structure_item $1) }
;
...
label_declaration:
    mutable_flag label COLON poly_type relations { ($2, $1, $4, $5, symbol_rloc()) }
;
